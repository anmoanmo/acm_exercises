...
vector<int> Get_point(int id,int op){//0->y 1->x
	...
	if(rev)for(int i=0;i<(int)now.size();i++)swap(now[i].a,now[i].b);
	vector<int>num;
	...
	for(int i=0;i<(int)now.size();i++){
		for(int j=i+1;j<(int)now.size();j++){
			...
			mid=min(mid,1ll*K);mid=max(mid,0ll);
			Ly=min(Ly,1ll*K);Ly=max(Ly,0ll);
			Ry=min(Ry,1ll*K);Ry=max(Ry,0ll);
			num.push_back(Ly);
			num.push_back(mid);
			num.push_back(Ry);
		}
...
vector<node>Get_val(vector<int>A,int id){
	vector<Lim_node>now=Lim[abs(id)];
	if(id<0)for(int i=0;i<(int)now.size();i++)swap(now[i].a,now[i].b);
	vector<node>res;
