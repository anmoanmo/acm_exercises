...
void radixSort(std::vector<int>& arr) {
    ...
    // 计算最大数字是几位数
    int maxLength = std::to_string(max).length();  // 使用 std::to_string 和 .length()
    // 定义一个二维数组，就是10个桶
    // 1. 该二维数组有10个一维数组 0-9
    // 2. 为了防止溢出，每个一维数组(桶)，大小定为 arr.size()
    // 3. 很明确, 基数排序是空间换时间
    std::vector<std::vector<int>> bucket(10, std::vector<int>(arr.size(), 0));
    // 用于记录在每个桶中，实际存放了多少个数据,这样才能正确的取出
    std::vector<int> bucketElementCounts(10, 0);
    // 根据最大长度的数决定比较的次数
    // 1. 大循环的次数就是 最大数有多少位,前面分析过
    // 2. n = 1, n *= 10 是为了每轮循环排序时，分别求出各个元素的个位，十位，百位，千位...
